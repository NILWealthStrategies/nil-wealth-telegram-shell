...
async function runTestScenario(scType) {
  ...
  const TEST_SUPPORT_SYS = "You write concise, structured support replies for NIL Wealth Strategies. Support tone must be formal, polished, complete, and executive-quality. Replies must fully answer every sender question before offering any next step. Answer fully using source-backed support knowledge and guide facts, while staying concise enough for email. Organization naming rule: use 'NIL Wealth Strategies' or 'Wealth Strategies'; never use standalone 'NIL' as the organization name. Mention Name, Image, and Likeness (NIL) only when the sender explicitly asks about Name, Image, and Likeness. If tax is asked, include a clear explanation of 1099 reporting, that Name, Image, and Likeness income is taxable, and practical next steps like tracking expenses and planning estimated taxes. If asked what supplemental health is, briefly explain it pays cash benefits for covered injuries/events to help with out-of-pocket costs like deductibles, copays, and coinsurance. Do not mention any insurer except Aflac. If carrier credibility is mentioned, use this fact pattern: Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families. Return strict JSON with v1, v2, v3, and rec.";

  const OUTREACH_SYS = "You write concise, human outreach replies for coach conversations. Tone must be conversational and relationship-building while still professional. Replies must directly answer every question the coach asked before suggesting a next step. If parent-group support is relevant, mention it only after the direct answer is clear. No corporate polish, no formal greetings, no hype language. Return JSON with v1,v2,v3 each containing subject and body.";
  ...
  const typePrompts = {
    ...
    PARENT_BASIC_QUESTION: `"s1": { "type": "PARENT_BASIC_QUESTION", "name": "<parent full name>", "email": "<parent email>", "school": "<high school>", "sport": "<sport>", "state": "<US state>", "subject": "<subject line>", "message": "<65-90 word parent question that feels real-world and cautious, asking whether coverage is optional, what it actually helps with, and whether it replaces current coverage. Do NOT name any insurer except Aflac if needed for credibility context; avoid all other insurer names.>" }`,
    OBJECTION_INSURANCE: `"s1": { "type": "OBJECTION_INSURANCE", "name": "<parent full name>", "email": "<parent email>", "school": "<high school>", "sport": "<sport>", "state": "<US state>", "subject": "<subject line>", "message": "<65-90 word parent objection that sounds realistic and skeptical, saying they already have coverage and asking what specific gap this solves and why they should care. Do NOT name any insurer except Aflac if needed for credibility context; avoid all other insurer names.>" }`,
  ...
  const scenarioAngleByType = {
    ...
    PARENT_BASIC_QUESTION: [
      "parent asks what this adds beyond existing health coverage",
      "parent asks if this is optional and who handles enrollment paperwork",
      "parent asks what situations trigger benefits and how families use them",
      "parent asks if they can review details quietly before deciding",
      "parent asks whether this is education support or direct insurance sales",
      "parent asks what the guide actually explains before any enrollment choice",
    ],
    OBJECTION_INSURANCE: [
      ...
      "parent rejects pressure language and wants plain facts only",
    ],
    REMOVAL_DEMAND: [
  ...
  if (scType === "OUTREACH_COACH_INTEREST") {
    ...
    v2 = result?.v2?.body || ""; v2subj = result?.v2?.subject || sc.subject;
    v3 = result?.v3?.body || ""; v3subj = result?.v3?.subject || sc.subject;
    checkBody = v2 || v1;
  } else {
    const typeRules = {
      ...
      OBJECTION_INSURANCE: "- V1 must acknowledge the parent has existing coverage (without naming it)\n- V1 must explain what gap supplemental coverage fills (deductibles, copays, out-of-pocket costs)\n- V1 must clarify this does not replace their existing plan\n- Leave the door open without pressure\n- Do not name any insurer except Aflac\n- If carrier credibility is mentioned, include: Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families",
    ...
    const result = JSON.parse(await askAI(TEST_SUPPORT_SYS,
      `Create 3 support reply drafts for this inbound conversation:\n${JSON.stringify({ contact_email: sc.email, subject: sc.subject, message: sc.message, school: sc.school, sport: sc.sport })}\n\nRules:\n- FORMAL tone — professional, complete sentences, warm but polished\n- Fully answer every sender question or concern before offering a next step\n- Answer fully using support knowledge and approved guide facts while staying concise\n- Use 'NIL Wealth Strategies' or 'Wealth Strategies' for organization naming; do not use standalone 'NIL' as the organization name\n- Mention Name, Image, and Likeness (NIL) only if the sender explicitly asks about Name, Image, and Likeness\n- If tax is asked, include a clear explanation of 1099 reporting, that Name, Image, and Likeness income is taxable, and practical next steps like tracking expenses and planning estimated taxes\n- If asked what supplemental health is, explain it pays cash benefits for covered injuries/events to help with out-of-pocket costs like deductibles, copays, and coinsurance\n- V1: direct and helpful\n- V2: warm and relationship-focused\n- V3: concise and executive-style\n- Under 185 words each\n- Do not mention AI\n- Do not name any insurer except Aflac\n- If carrier credibility is mentioned, include: Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families\n${typeRules}\nAlso include a recommendation.\nReturn: {"v1":{"subject":"...","body":"..."},"v2":{"subject":"...","body":"..."},"v3":{"subject":"...","body":"..."},"rec":{"include_link":"yes|no","guide":"parent-guide|supplemental-health-guide|none"}}`,
      true
    ));
    v1 = result?.v1?.body || ""; v1subj = result?.v1?.subject || sc.subject;
    v2 = result?.v2?.body || ""; v2subj = result?.v2?.subject || sc.subject;
  ...
  if (scType === "OUTREACH_COACH_INTEREST") {
    try {
      const ccSys = "You generate CC Support messages for NIL Wealth Strategies. Bridge: concise, conversational, polished note from outreach person to coach looping in support. The bridge must explicitly tell the coach that the note below is what they can forward to the parent group. Support: formal, persuasive, executive-quality message the coach forwards to parent group. The support message must build information in the right order: context, what families need to know, role clarity, why the Parent Guide is worth opening, then the response line. Answer every question fully using source-backed support knowledge and guide facts while remaining concise. Use 'NIL Wealth Strategies' or 'Wealth Strategies' when naming the organization; do not use standalone 'NIL' as the organization name. Mention Name, Image, and Likeness (NIL) only if the sender explicitly asks about Name, Image, and Likeness. Do not name any insurer except Aflac. If carrier credibility is mentioned, include that Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families. Return JSON.";
      const ccResult = JSON.parse(await askAI(ccSys,
        `Generate CC messages for this coach conversation:\nCoach: ${sc.name} — ${sc.school} ${sc.sport} (${sc.state})\nMessage: ${sc.message}\n\nBridge (concise, conversational, professional; under 65 words; outreach person says support team is looped in; explicitly say the note below is what the coach can forward to the parent group; do not repeat the coach name in the bridge body):\nSupport (formal, under 205 words, written to be forwarded to the parent group — first give parents clear context, then explain what the program covers, then clarify roles in fluent professional wording, then explain why the Parent Guide is worth opening by telling families what they will find there, then include this exact line: "You can respond to this message with any questions — we're happy to help.", and include: "Learn more in the Parent Guide:" on one line followed by this link on the next line: https://mynilwealthstrategies.com/access-parent-guide):\n\nReturn: {"bridge":{"body":"..."},"support":{"body":"..."}}`,
        true
      ));
      ccBridge = ccResult?.bridge?.body || "";
      ccSupport = ccResult?.support?.body || "";
    } catch {}
  } else if (scType === "OBJECTION_INSURANCE") {
    try {
      const ccSys = "You generate CC Support responses for NIL Wealth Strategies. Formal, professional, polished, and persuasive. The message must answer the parent's concern clearly, build trust step by step, and explain why the Parent Guide is worth opening in a credible way. Answer every question fully using source-backed support knowledge and guide facts while remaining concise. Use 'NIL Wealth Strategies' or 'Wealth Strategies' when naming the organization; do not use standalone 'NIL' as the organization name. Mention Name, Image, and Likeness (NIL) only if the sender explicitly asks about Name, Image, and Likeness. Do not name any insurer except Aflac. If carrier credibility is mentioned, include that Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families. Return JSON.";
      const ccResult = JSON.parse(await askAI(ccSys,
        `Generate a CC Support response for a parent who said they feel well-covered and question the gap:\nParent: ${sc.name}\nMessage: "${sc.message}"\n\nWrite a formal, warm CC support message (under 205 words) that:\n1) Acknowledges they have existing health coverage\n2) Clearly answers what gap supplemental coverage is meant to address\n3) States this is optional and family-driven\n4) States coaches do not sell, explain in detail, or enroll insurance\n5) States coaches do not handle money or paperwork\n6) States families review options and enroll directly with Aflac\n7) States NIL Wealth Strategies provides education and support only\n8) Includes own-pace wording (families can move at their own pace)\n9) Gives a credible, specific reason to open the Parent Guide by explaining what families will find in it\n10) Includes this exact line: "You can respond to this message with any questions — we're happy to help."\n11) Includes: "Learn more in the Parent Guide:" on one line and this link on the next line: https://mynilwealthstrategies.com/access-parent-guide\n\nReturn: {"body":"..."}`,
        true
      ));
  ...
  const failures = {
    OUTREACH_COACH_INTEREST: () => [
      ...
      wcount(checkBody) <= 130 ? null : `Over word limit — ${wcount(checkBody)} words (max 130)`,
    ...
    OBJECTION_INSURANCE: () => [
      ...
      contains(checkBody, "understand", "appreciate", "while those", "already have", "valid", "reasonable", "your existing", "your current", "primary", "current coverage", "existing policy", "current policy", "such as your", "alongside your") ? null : "Did not acknowledge existing coverage or concern",
      ...
      contains(checkBody, "not replace", "supplement", "alongside", "additional") ? null : "Did not clarify it does not replace existing insurance",
  ...
  // ── Page 3: Drafts card V1 / V2 / V3 (real drafts card format) ─
  pages.push([
    ...
    `▫️ V1`,
    `Subject: ${escT(v1subj)}`,
    testTrunc(escT(v1), 340),
    ``,
    `▫️ V2`,
    `Subject: ${escT(v2subj)}`,
    testTrunc(escT(v2), 340),
    ``,
    `▫️ V3`,
    `Subject: ${escT(v3subj)}`,
    testTrunc(escT(v3), 340),
  ].join("\n"));
  ...
  // ── Page 4/5 (Objection): CC Support gap response ────────────
  if (scType === "OBJECTION_INSURANCE" && ccSupport) {
    pages.push([
      `📌 CC SUPPORT`,
      `--`,
      `${convId} · 📧 Support`,
      `${escT(sc.name)}`,
      ``,
      `Gap coverage response:`,
      `──`,
      testTrunc(escT(ccSupport), 620),
      ``,
      `🧪 SIMULATED CC`,
    ].join("\n"));
  }
...
const programsSystemPrompt = "You write concise, human outreach replies for coach conversations. The sender is personal, mission-driven, and sounds like a real person, not a sales rep. Hard tone rule: outreach should be conversational and relationship-building while still professional. No corporate polish, no stiff formal greetings, no structured paragraphs, and no slangy hype language. Insurance mention rule: do not name any insurer except Aflac. If carrier credibility is mentioned, use this fact pattern: Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families. Return JSON with v1,v2,v3 each containing subject and body."; 
const supportSystemPrompt = "You write concise, structured support replies. Hard tone rule: support must be FORMAL — professional, organized, clear, and complete sentences. Not casual slang, not text-message style. Warm but polished. Fully answer every sender question using approved support knowledge and guide facts, while remaining concise. Organization naming rule: use 'NIL Wealth Strategies' or 'Wealth Strategies'; never use standalone 'NIL' as the organization name. Mention Name, Image, and Likeness (NIL) only if the sender explicitly asks about Name, Image, and Likeness. If tax is asked, include a clear explanation of 1099 reporting, that Name, Image, and Likeness income is taxable, and practical next steps like tracking expenses and planning estimated taxes. If asked what supplemental health is, explain it pays cash benefits for covered injuries/events to help with out-of-pocket costs like deductibles, copays, and coinsurance. Insurance mention rule: do not name any insurer except Aflac. If carrier credibility is mentioned, use this fact pattern: Aflac holds an AM Best financial strength rating of A+ (Superior), and coaches including Deion Sanders, Nick Saban, and Dawn Staley have publicly endorsed Aflac's mission of protecting families. Return JSON with v1,v2,v3 each containing subject and body.";
