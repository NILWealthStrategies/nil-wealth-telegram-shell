...
async function runTestScenario(scType) {
  ...
  if (scType === "OUTREACH_COACH_INTEREST") {
    ...
    const outboundSeed = `Hey Coach ${escT(coachLastName)} - I am with NIL Wealth Strategies. I am a former D1 athlete, and during my college career I had 3 surgeries, so I saw firsthand how fast out-of-pocket expenses can stack up after an injury. Our service is prioritized for high school athletes because parents often have to handle these cost gaps on their own. Primary insurance usually requires deductibles, copays, and coinsurance to be paid first before it covers the full medical expenses, which can leave families exposed. That is why we focus on simple supplemental health education, risk awareness, and tax education so families can make clear decisions without pressure. Is this the right person to get in contact with the families to help athletes?`;
    pages.push([
      `📤 INSTANTLY OUTBOUND`,
      `--`,
      `${escT(sc.name)}`,
