index.js — NIL Wealth Telegram Ops Shell (v3.5 FULL FILE)

/** 
 * NIL Wealth Telegram Ops Shell — SUPABASE OPS (Index.js v3.5) 
 * 
 * v3.5 updates (ONLY what's necessary): 
...
function replyLabel(mode) { 
  return mode === "support" ? "󰼡 Support" : "🏈 Outreach"; 
} 
...
function startOfNYWeekISO(date = new Date()) { 
  ...
  const weekStart = new Date(`${p2.month}/${p2.day}/${p2.year} 00:00:00 
GMT-0500`); 
...
function normalizeSubmissionRow(s) { 
  ...
  const submission_id = s.submission_id || s.id || s.submissionId || 
s.submissionID; 
  const payload = s.submission_payload || s.payload || s.form_payload || s.data || 
null; 

  // Status fields (your spec uses: email_sent / text_sent and email_error / 
text_error) 
  const email_sent = 
  ...
  return { 
    ...
    submission_payload: payload && typeof payload === "string" ? 
safeJsonParse(payload) : payload, 
...
async function sbMetricsSummary({ scope = "company", sinceIso } = {}) { 
  const programLinkOpens = await sbCountMetric("program_link_open", { scope, 
sinceIso }); 
  const coverageExploration = await sbCountMetric("coverage_exploration", { scope, 
sinceIso }); 

  const parentGuideClicks = await sbCountMetric("parent_guide_click", { scope, 
sinceIso }); 
  ...
  const taxEducationClicks = await sbCountMetric("tax_education_click", { scope, 
sinceIso }); 
  ...
  const riskAwarenessClicks = await sbCountMetric("risk_awareness_click", { scope, 
sinceIso }); 
  const shClicks = await sbCountMetric("sh_click", { scope, sinceIso }); 

  const websiteSubmissions = await sbCountMetric("website_submission", { scope, 
sinceIso }); 
...
function applyLocalDueFilters(rows, f) { 
  ...
  if (f.dueToday) 
    return rows.filter((r) => minsUntilUrgent(r.updated_at || r.created_at) <= 24 
* 60); 
  return rows; 
} 
...
// v3.5: lock-screen safe notification (no data shown) 
...
async function buildConversationText(conv) { 
  ...
  const createdLine = conv.created_at ? `⏱ Created: 
${fmtISOShort(conv.created_at)}` : ""; 
  const updatedLine = conv.updated_at ? `⏱ Updated: 
${fmtISOShort(conv.updated_at)}` : ""; 
  const doneLine = conv.completed_at ? `✅ Done: 
${fmtISOShort(conv.completed_at)}` : ""; 
  const tsBlock = [createdLine, updatedLine, doneLine].filter(Boolean).join("\n"); 

  const urgentBadge = conv.urgent ? "‼ URGENT" : ""; 
  const laneLine = `${laneLabel(sourceSafe(conv.source))}`; 

  const lockLine = conv.owned_by 
    ? `🔒 Owned by ${conv.owned_by === "support" ? "󰼡 Support" : "🏈 Outreach"}` 
    : ""; 

  const summary = await buildConversationSummary(conv); 
  const summaryBlock = [ 
    `💬 Messages: ${summary.msgCount}`, 
    summary.lastInboundPreview ? `⬅ Inbound: ${summary.lastInboundPreview}` : 
null, 
    summary.lastOutboundPreview ? `➡ Outbound: ${summary.lastOutboundPreview}` : 
null, 
  ] 
    .filter(Boolean) 
    .join("\n"); 
...
function buildConversationKeyboard(conv) { 
  ...
  const gmailBtn = conv.gmail_url ? Markup.button.url("📬 Open in Gmail", 
conv.gmail_url) : null; 
...
function buildSubmissionText(sub) { 
  ...
  const createdLine = sub.created_at ? `⏱ Created: 
${fmtISOShort(sub.created_at)}` : ""; 

  const payloadBlock = 
formatSubmissionPayloadBlockFromPayload(sub.submission_payload); 

  const deliveryLines = []; 
  deliveryLines.push(`Email Sent ${sentBadge(sub.email_sent)}`); 
  deliveryLines.push(`Text Sent ${sentBadge(sub.text_sent)}`); 
  if (sub.email_error) deliveryLines.push(`Email Error: ${shorten(sub.email_error, 
140)}`); 
  if (sub.text_error) deliveryLines.push(`Text Error: ${shorten(sub.text_error, 
140)}`); 
...
function oneLineSummary(conv, idx) { 
  ...
  const lane = sourceSafe(conv.source) === "support" ? "󰼡 Support" : "🏈 
Programs"; 
...
async function showQueueSummaryList(ctx, key, rows, filterSource) { 
  ...
  if (!rows.length) { 
    await ctx.reply( 
      `${header}\n\n(None right now)`, 
      Markup.inlineKeyboard([[Markup.button.callback("⬅ Back", "DASH:back")]]) 
    ); 
    return; 
  } 
  ...
  const kbRows = isSubmissions 
    ? rows.slice(0, 8).map((c, i) => [Markup.button.callback(`Open ${i + 1}`, 
`OPENCARD:${c.id}`)]) 
...
function formatMessageLineFull(m) { 
  const who = m.direction === "inbound" ? "⬅ Inbound" : "➡ Outbound"; 
...
Messages: ${total}\nShowing: ${ 
  ...
  const kb = Markup.inlineKeyboard([ 
    [ 
      ...(hasPrev 
        ? [Markup.button.callback("◀ Older", `THREAD:${convId}:${prevOffset}`)] 
        : []), 
      ...(hasNext 
        ? [Markup.button.callback("▶ Newer", `THREAD:${convId}:${nextOffset}`)] 
        : []), 
    ], 
    [Markup.button.callback("⬅ Back to Card", `OPENCARD:${convId}`)], 
...
async function dashboardCounts(filterSource = "all") { 
  const urgentCount = await sbCountConversations({ pipeline: "urgent", source: 
filterSource }); 
  const needsReplyCount = await sbCountConversations({ pipeline: "needs_reply", 
source: filterSource }); 
  const waitingCount = await sbCountConversations({ pipeline: "actions_waiting", 
source: filterSource }); 
  const activeCount = await sbCountConversations({ pipeline: "active", source: 
filterSource }); 
  const followCount = await sbCountConversations({ pipeline: "followups", source: 
filterSource }); 
  const forwardedCount = await sbCountConversations({ pipeline: "forwarded", 
source: filterSource }); 
...
function dashboardKeyboard(_filterSource = "all") { 
  ...
  const supBtn = Markup.button.callback("󰼡 Support", "FILTER:support"); 
...
async function todayCounts(filterSource = "all") { 
  const urgent = await sbCountConversations({ pipeline: "urgent", source: 
filterSource }); 
  const needs = await sbCountConversations({ pipeline: "needs_reply", source: 
filterSource }); 
  ...
  const waiting = await sbCountConversations({ pipeline: "actions_waiting", 
source: filterSource }); 
  const active = await sbCountConversations({ pipeline: "active", source: 
filterSource }); 
  const followups = await sbCountConversations({ pipeline: "followups", source: 
filterSource }); 

  // ✅ Submissions MUST come from ops.submissions 
  const submissions = filterSource === "programs" ? 0 : await 
sbCountSubmissions(); 
...
bot.start(async (ctx) => { 
  await ctx.reply( 
    `✅ Connected.\n${CODE_VERSION} · Build: ${String(BUILD_VERSION).slice(0, 
8)}\nType /dashboard` 
  ); 
}); 
...
bot.command("dashboard", async (ctx) => { 
  ...
  await ctx.reply(await dashboardText(filterSource), 
dashboardKeyboard(filterSource)); 
}); 
...
bot.command("search", async (ctx) => { 
  ...
  if (!q) { 
    await ctx.reply( 
      `🔎 Search usage:\n/search your text\n/search coach:ABC source:support 
...
bot.action("DASH:back", async (ctx) => {
  ...
  // v3.5: Dashboard button always returns to ALL filter
  setAdminFilter(ctx, "all");
  await ctx.reply(await dashboardText("all"), dashboardKeyboard("all"));
}); 
...
/^VIEW:(urgent|needs_reply|actions_waiting|active|followups|forwarded|website_subm
issions)$/, 
  async (ctx) => { 
    if (!isAdmin(ctx)) return; 
    await ctx.answerCbQuery(); 
    ...
    if (key === "website_submissions") { 
      const rows = filterSource === "programs" ? [] : await sbListSubmissions({ 
limit: 8 }); 
      await showQueueSummaryList(ctx, "website_submissions", rows, filterSource); 
      return; 
    } 

    const rows = await sbListConversations({ pipeline: key, source: filterSource, 
limit: 8 }); 
...
// Open card: try conversation first, else treat as submission_id in 
ops.submissions 
bot.action(/^OPENCARD:(.+)$/, async (ctx) => { 
  ...
  if (conv) { 
    const msg = await ctx.reply(await buildConversationText(conv), 
buildConversationKeyboard(conv)); 
  ...
  const msg = await ctx.reply(buildSubmissionText(sub), 
buildSubmissionKeyboard(sub.submission_id)); 
...
bot.action(/^OPENMIRROR:(.+)$/, async (ctx) => { 
  ...
  const convId = ctx.match[1]; 
  const conv = await sbGetConversation(convId); 
  if (!conv || !conv.mirror_conversation_id) return ctx.reply("No mirror 
linked."); 
  ...
  const msg = await ctx.reply(await buildConversationText(mirror), 
buildConversationKeyboard(mirror)); 
...
async function createSupportMirrorConversation(programConv) { 
  ...
  const mirrorId = await sbUpsertConversationByThreadKey(mirrorThreadKey, { 
    ...
    subject: programConv.subject ? `🪞 ${programConv.subject}` : "🪞 Support 
Mirror", 
  ...
  try { 
    if (kind === "conv") { 
      try { 
        ...
        await 
supabase.schema("ops").from("messages").delete().eq("conversation_id", id); 
      } catch (_) {} 

      const { error } = await 
supabase.schema("ops").from("conversations").delete().eq("id", id); 
  ...
  for (const [messageId, meta] of liveCards.entries()) { 
    if (kind === "conv" && meta.type === "conversation" && String(meta.ref_id) === 
String(id)) { 
      liveCards.delete(messageId); 
    } 
    if (kind === "sub" && meta.type === "submission" && String(meta.ref_id) === 
String(id)) { 
      liveCards.delete(messageId); 
    } 
  } 
...
bot.action(/^SEND:(.+):([01])$/, async (ctx) => { 
  ...
  if (src === "support") { 
    await ctx.reply( 
      ...
      Markup.inlineKeyboard([ 
        [Markup.button.callback("✅ Yes, Send", `DOSEND:${convId}:${cc ? 1 : 
0}:support`)], 
        [Markup.button.callback("⬅ Cancel", "DASH:back")], 
      ]) 
    ); 
  ...
  if (owned === "support" || owned === "outreach") { 
    const fixed = owned; 
    await ctx.reply( 
      `🔒 Owned by ${fixed === "support" ? "󰼡 Support" : "🏈 Outreach"}\nSend 
as: ${replyLabel( 
        fixed 
      )}\n\nAre you sure you want to send?`, 
      Markup.inlineKeyboard([ 
...
0}:${fixed}`)], 
  ...
  await ctx.reply( 
    ...
    Markup.inlineKeyboard([ 
      [ 
        Markup.button.callback("🏈 Outreach", `CONFIRMSEND:${convId}:${cc ? 1 : 
0}:outreach`), 
        Markup.button.callback("󰼡 Support", `CONFIRMSEND:${convId}:${cc ? 1 : 
0}:support`), 
      ], 
      [Markup.button.callback("⬅ Cancel", "DASH:back")], 
    ]) 
    ...
    if (!mirrorId) { 
      ...
      if (refreshed) { 
        ...
        if (refreshed.mirror_conversation_id) { 
          await sbUpdateConversation(refreshed.mirror_conversation_id, { owned_by: 
"support" }); 
        } 
      } 
  ...
  const cooldown = new Date(nowMs() + URGENT_COOLDOWN_HOURS * 60 * 60 * 
1000).toISOString(); 
  ...
  if (conv.coach_id) { 
    await sbUpsertCoach({ coach_id: conv.coach_id, coach_name: conv.coach_name || 
null }); 
...
async function urgentLoop() { 
  ...
  for (const c of rows || []) { 
    ...
    const lastNotified = c.last_notified_at ? new 
Date(c.last_notified_at).getTime() : 0; 
    ...
    try { 
      ...
      if (ms - lastNotified > notifyCooldownMs) { 
        // v3.5: lock-screen safe notification 
...
") 
    .eq("source", "support") 
    .in("pipeline", ["active", "needs_reply", "actions_waiting", "followups", 
"urgent"]) 
    .order("updated_at", { ascending: true }) 
    .limit(250); 
...
function fmtCoachLine(c, followupCount = null) { 
  ...
  return `${name}${prog}\n✅ Sent: ${sent} · 💬 Replies: ${reps} · 🔗 Clicks: 
${clicks} · 📨 Forwards: ${fwd}${fu}`; 
} 
...
clicks/forwards/people come in, they will appear here.`, 
      Markup.inlineKeyboard([[Markup.button.callback("⬅ Back", "DASH:back")]]) 
    ); 
    return; 
  } 

  const lines = coaches.map((c, i) => `${i + 1}) 
${fmtCoachLine(c)}`).join("\n\n"); 
...
async function showCoachOverview(ctx, coach_id) { 
  ...
  if (!coach) { 
    await ctx.reply( 
      "Coach not found.", 
      Markup.inlineKeyboard([[Markup.button.callback("⬅ Back", "CALLS:hub")]]) 
    ); 
    return; 
  } 
...
: "—"}`; 
  ...
  const kb = Markup.inlineKeyboard([ 
    ...
    [ 
      ...
      Markup.button.callback("📝 Needs Reply", 
`COACHVIEW:${coach_id}:needs_reply`), 
    ], 
    [Markup.button.callback("⬅ Back", "CALLS:hub")], 
  ]); 
...
${people.length}`; 
  ...
  if (!people.length) { 
    await ctx.reply( 
      `${title}\n\n(No people yet for this coach)\n\nPeople appear here from 
/webhook/person or when you choose to attach them in automation.`, 
      Markup.inlineKeyboard([[Markup.button.callback("⬅ Back", 
`COACH:${coach_id}`)]]) 
    ); 
    return; 
  } 
  ...
  const kb = Markup.inlineKeyboard([ 
    ...
    [Markup.button.callback("⬅ Back", `COACH:${coach_id}`)], 
  ]); 
...
"—"}`; 
  ...
  await ctx.reply( 
    text, 
    Markup.inlineKeyboard([ 
      [Markup.button.callback("⬅ Back to People", `PEOPLE:${p.coach_id}`)], 
      [Markup.button.callback("⬅ Coach", `COACH:${p.coach_id}`)], 
      [Markup.button.callback("🏠 Dashboard", "DASH:back")], 
    ]) 
  ); 
} 
...
async function showFollowupsGroupedByCoach(ctx, filterSource) { 
  ...
  for (const r of rows) { 
    if (filterSource !== "all" && sourceSafe(r.source) !== 
sourceSafe(filterSource)) continue; 
  ...
  for (const id of coachIds) { 
    ...
    coaches.push( 
      c || { 
        coach_id: id, 
        coach_name: rows.find((x) => (x.coach_id || "unknown") === id)?.coach_name 
|| id, 
      } 
    ); 
  } 
  ...
  const kb = Markup.inlineKeyboard([ 
    ...coaches.map((c, i) => [ 
      Markup.button.callback(`${i + 1}) Open Coach`, `COACH:${c.coach_id}`), 
      Markup.button.callback("📚 Follow-Ups", 
`COACHVIEW:${c.coach_id}:followups`), 
    ]), 
    [Markup.button.callback("⬅ Back", "DASH:back")], 
  ]); 
 ...
 * Supports: 
 * - pipeline: 
urgent|needs_reply|actions_waiting|active|followups|forwarded|completed|website_su
bmissions 
 ...
 * 
 * website_submissions writes to ops.submissions (source of truth for Submissions 
UI) 
...
app.post("/webhook/item", async (req, res) => { 
  try { 
    ...
    const effectivePipeline = pipeline || (dir === "inbound" ? "needs_reply" : 
"active"); 

    // ✅ If this is a website submission, write to ops.submissions (source of 
truth) 
    if (effectivePipeline === "website_submissions") { 
      ...
      const finalEmailSent = typeof email_sent === "boolean" ? email_sent : 
undefined; 
...
supabase.schema("ops").from("submissions").insert({ 
      ...
      // Optional: also store a message in conversations for audit if you already 
rely on it elsewhere. 
      // (Does NOT affect Submissions UI.) 
      try { 
        const convId = await 
sbUpsertConversationByThreadKey(`submission:${submissionId}`, { 
          source: "support", 
          pipeline: "website_submissions", 
          subject: `🧾 Website Submission · ${payloadObj?.name || 
payloadObj?.email || "New"}`, 
          preview: preview || shorten(body || JSON.stringify(payloadObj || {}), 
220), 
          created_at: isoNow(), 
        }); 
...
: ""); 
      ...
      notifyAdmins( 
        ...
        { 
          keyboard: Markup.inlineKeyboard([ 
            [Markup.button.callback("Open 🧾 Submissions", 
"VIEW:website_submissions")], 
            [Markup.button.callback("📅 Today", "TODAY:open")], 
          ]), 
        } 
      ).catch(() => {}); 
...
app.post("/webhook/click", async (req, res) => { 
  try { 
    ...
    if (!coach_id) return res.status(400).json({ ok: false, error: "coach_id 
required" }); 
...
app.post("/webhook/person", async (req, res) => { 
  try { 
    ...
    const { coach_id, name, email, phone, status, summary, meta, last_activity_at, 
coach_name } = 
      req.body || {}; 
    if (!coach_id) return res.status(400).json({ ok: false, error: "coach_id 
required" }); 
    ...
    if (!id) 
      return res.status(500).json({ ok: false, error: "people upsert failed (table 
missing?)" }); 
...
app.post("/webhook/failure", async (req, res) => { 
  ...
  try { 
    ...
    if (error) { 
      await notifyAdmins(`⚠ FAILURE (db insert 
failed)\n${row.kind}\n${row.message}`); 
      return res.json({ ok: false, error: error.message }); 
    } 

    await notifyAdmins(`⚠ FAILURE INBOX\n${row.kind}\n${row.message}`); 
    return res.json({ ok: true }); 
  } catch (e) { 
    return res.status(500).json({ ok: false, error: String(e.message || e) }); 
  } 
