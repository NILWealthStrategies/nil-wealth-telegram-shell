...
async function runTestScenario(scType) {
  ...
  if (scType === "OUTREACH_COACH_INTEREST") {
    ...
    const outboundSeed = `Hey Coach ${escT(coachLastName)} - I am with NIL Wealth Strategies. We help student-athletes and families understand financial risks, how NIL income is taxed, and how to plan for injury-related costs in clear practical terms. I am a former D1 athlete, and during my college career I went through three surgeries, so I saw firsthand how quickly out-of-pocket costs can stack up after an injury. Because of that, we prioritize high school athletes for injury expense coverage since parents are often left handling gaps that primary insurance does not fully cover. Is this the right person to connect with families and help athletes?`;
    pages.push([
      `📤 INSTANTLY OUTBOUND`,
      `--`,
      `${escT(sc.name)}`,
